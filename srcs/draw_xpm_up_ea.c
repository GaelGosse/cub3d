...
/*   Updated: 2023/08/03 12:47:15 by gael             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_xpm_up_ea(t_game *game, int i_midline)
{
	...
	if (perc_y != 0 && perc_y < game->xpm->ea_width_height)
		...
		roundf(game->line_3d->corr_y), \
		draw_xpm_up_ea_color(game, perc_x, perc_y));
}
...
int	draw_xpm_up_ea_color(t_game *game, int x, int y)
{
	...
	{
		if (y + game->xpm->ea_tab_start < game->xpm->ea_width_height && \
		game->xpm->ea_colors[i_find_c][0] == \
